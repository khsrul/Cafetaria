...
class SoldModel extends Model
{
    ...
    public function item_sold() {
        $this->select('invoices.invoice_id, invoices.invoice, tables.table_name, menus.menu_name, menus.price, menus.category, menus.menu_pic, orders.quantity, invoices.total, invoices.date_order');
    ...
    public function most_item_sold($id = null) {
        $this->select('invoices.invoice_id, invoices.invoice, tables.table_name, menus.menu_name, menus.price, menus.category, menus.menu_pic, orders.quantity, invoices.total, invoices.date_order');
        $this->where('menus.category', $id);
        $this->join('orders', 'invoices.order_id = orders.order_id');
        $this->selectSUM('orders.quantity');
        $this->join('menus', 'orders.menu_id = menus.menu_id');
        $this->join('tables', 'orders.table_id = tables.table_id');
